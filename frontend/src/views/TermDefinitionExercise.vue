...
function isOptionDisabled(optionId) {
    return loading.value || isCorrect.value ||
        (showCorrectAnswer.value && attempts.value >= 2)
}
            ...
            <div v-if="exercise" class="p-6 rounded-lg shadow-lg transition-all duration-300" :class="[
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <button v-for="option in exercise.result.content.options" :key="option.id"
                        @click="selectAnswer(option.id)" :disabled="isOptionDisabled(option.id)"
                        ...
                        :class="[
                            ...
                            showCorrectAnswer && exercise.result.correct_answer === option.id
                                ...
                                : '',
                            showCorrectAnswer && selectedAnswer === option.id && exercise.result.correct_answer !== option.id
                                ? 'ring-2 ring-red-500'
                                : '',
                ...
                <div v-if="attempts === 1 && !isCorrect"
                    class="mt-4 p-4 rounded-lg transition-all duration-300 animate-fade-in" :class="[
                        themeStore.isDark

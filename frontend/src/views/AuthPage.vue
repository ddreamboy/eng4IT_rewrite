...
<template>
  ...
  >
    <div
      class="w-full max-w-md p-6 m-4 rounded-lg shadow-lg"
      :class="[themeStore.isDark ? 'bg-dark-secondary' : 'bg-light-secondary']"
    >
      <!-- Переключатель форм -->
      <div
        class="flex p-1 mb-6 rounded-lg"
        :class="[themeStore.isDark ? 'bg-dark-primary' : 'bg-light-primary']"
      >
        <button
      ...
      <form @submit.prevent="handleSubmit" class="space-y-4">
        ...
        <div v-if="activeTab === 'register'" class="space-y-1">
          ...
          <input
            ...
            type="text"
            class="w-full p-2 rounded border transition-colors"
            :class="[
              themeStore.isDark
        ...
        <button
          type="submit"
          class="w-full py-2 text-white rounded-lg transition-colors disabled:opacity-50"
          :class="[
            themeStore.isDark

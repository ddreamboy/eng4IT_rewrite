// src/App.vue
<template>
  <div class="min-h-screen transition-colors duration-200" :class="[
    themeStore.isDark
    ...
    <template v-if="!isAuthPage">
      <!-- Desktop Navigation -->
      <nav class="hidden md:flex h-16 items-center justify-between px-6 shadow-md"
      ...
      <!-- Mobile Header -->
      <nav class="md:hidden flex h-16 items-center justify-between px-4"
        :class="[themeStore.isDark ? 'bg-dark-secondary' : 'bg-light-secondary']">
        <RouterLink to="/" class="text-xl font-bold transition-colors"
          :class="[themeStore.isDark ? 'text-dark-accent' : 'text-light-accent']">
          Eng4IT
        </RouterLink>
        <div class="flex items-center space-x-4">
...
watchEffect(() => {
  // Обновляем классы на документе при изменении темы
